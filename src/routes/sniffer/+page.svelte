...
<div class="h-full flex flex-col relative bg-zinc-950">
  ...
  <button class="absolute right-8 bottom-8 w-14 h-14 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-full shadow-xl flex items-center justify-center z-40 group" onclick={() => showDrawer = !showDrawer}>
  ...
  {#if showDrawer}
    <div class="absolute inset-0 bg-black/60 backdrop-blur-sm z-40" onclick={() => showDrawer = false}></div>
    <div class="absolute inset-x-0 bottom-0 h-[30rem] bg-zinc-900 border-t border-zinc-700 shadow-2xl flex flex-col z-50">
      <div class="flex justify-between items-center p-4 border-b border-zinc-800/50">
        ...
        <button onclick={() => showDrawer = false} class="text-zinc-500 hover:text-zinc-300"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
      </div>
